...
class Toast {
    ...
    func present(title: String, symbol: String?, tintTitle: Color = .primary, tintSymbol: Color = .primary, isUserInteractionEnabled: Bool = false, timing: ToastTime = .medium) {
        withAnimation(.snappy) {
            toasts.append(.init(title: title, symbol: symbol, tintTitle: tintTitle, tintSymbol: tintSymbol, isUserInteractionEnabled: isUserInteractionEnabled, timing: timing))
        }
    }
}
        ...
        HStack(spacing: 0) {
            if let symbol = item.symbol {
                Image(systemName: symbol)
                    .font(.title3)
                    .foregroundStyle(item.tintSymbol)
                    .padding(.trailing, 10)
            }
            
            Text(item.title)
                .font(.footnote)
                .foregroundStyle(item.tintTitle)
                .lineLimit(1)
        }
    //   .foregroundStyle(item.tint)
        .padding(.horizontal, 15)
        .padding(.vertical, 8)
        .background(ColorManager.toastBg)
        .background(
            .background
